...
dfSOCIAL <- dfTPPCOMPARE %>% 
  select(Seat, state, PartyAb.22, PartyAb.19, ALP.change, ALP.change.ppt, seat.type, Swing.22)  %>% 
  #rename(Seat = Seat, swing = Swing.22) %>%
  #mutate(swing = swing * -1) %>%
  inner_join(dfSA1 %>% select(Seat, richSA1s_pc), by = 'Seat') %>%
  filter(state == 'VIC')
  
  #mutate(state_group = ifelse(state %in% c('VIC'), 'GROUP', 'OTHER')) 

###############

dfSOCIAL %>% ggplot(aes(richSA1s_pc, Swing.22, color=state)) + geom_point(size = 5, alpha=0.8)


decile_cols <- c("Decile.1.SA1s","Decile.2.SA1s", "Decile.3.SA1s","Decile.4.SA1s","Decile.5.SA1s","Decile.6.SA1s","Decile.7.SA1s","Decile.8.SA1s","Decile.9.SA1s","Decile.10.SA1s")


makeBPs <- function(cols, angle) {
  dfx <- dfSA1[cols] %>% melt()
  dfx %>% ggplot(aes(variable, value, fill = variable)) + 
    geom_boxplot() + 
    theme_classic() + 
    theme(legend.position = "none", axis.text.x = element_text(angle=angle, hjust=1, vjust=0.5), text=element_text(size=6,  family="Inter", color='black')) 
}
makeBPs(decile_cols, 90)
