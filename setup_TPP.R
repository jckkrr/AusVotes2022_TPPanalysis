...
dfTPPCOMPARE <- dfTPPCOMPARE %>% mutate(TPP.swap = ifelse(PartyAb.22 != PartyAb.19, paste(PartyAb.22, 'gain'), 'no change'))
...
### I also want to add enrolment , turnout and informality, which is stored in a seperate CSV
dfENROL <- read.csv('enrolment_etc.csv')
dfTPPCOMPARE <- dfTPPCOMPARE %>% inner_join(dfENROL, by='seat')
dim(dfTPPCOMPARE)
names(dfTPPCOMPARE)
